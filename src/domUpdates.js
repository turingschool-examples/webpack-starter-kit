...
export default {
  ...
  domAllServicesOfOneDay(services) {
    ...
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="header_services">Services</th><th class="header_quantity">Quantity</th><th class="header_price">Total price</th></tr>`
    );
    for (let i = 0; i < data.length; i++) {
      elements = elements.add(
        `<tr><td>${i + 1}</td><td>${data[i][0]}</td><td class="align-center">${
          data[i][1][0]
        }</td><td class="align-right">${data[i][1][1]}</td></tr>`
      );
    }
    ...
    if (data.length !== 0) {
      ...
      $(".services-general-info-services").append(
        `<p class="prompt">No services for that date...</p>`
      );
      $(".services-title").text("");
    }
  },
  ...
  domMostPopularDay(data) {
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="days">Most popular day/s (${
        data.numOfDays
      }) </th></tr>`
    );
  ...
  domLeastPopularDay(data) {
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="days">Least popular day/s (${
        data.numOfDays
      }) </th></tr>`
    );
  ...
  domSearchCustomerName(filtredCustomers) {
    ...
    for (let i = 0; i < filtredCustomers.length; i++) {
      elements = elements.add(
        `<li data-id="${filtredCustomers[i].id}" class="names-found">${
          filtredCustomers[i].name
        }</li>`
      );
    }
  ...
  domRoomsAvailable(rooms) {
    ...
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="room-type">Room type</th><th class="available-rooms">Available rooms</th><tr>`
    );
    for (let i = 1; i < data.length; i++) {
      elements = elements.add(
        `<tr><td>${i}</td><td>${data[i][0]}</td><td class="align-center">${
          data[i][1]
        }</td></tr>`
      );
    }
    elements = elements.add(
      `<tr><td colspan="2">Total rooms available</td><td class="align-center">${
        data[0][1]
      }</td></tr>`
    );
  ...
  domCustomerBookingHistory(bookingHistory) {
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="room-type">Room type</th><th class="available-rooms">Date</th><th class="cancel-booking-header">Cancel booking</th><tr>`
    );

    for (let i = 0; i < bookingHistory.length; i++) {
      elements = elements.add(
        `<tr><td>${i + 1}</td><td>${
          bookingHistory[i].roomType
        }</td><td class="align-center">${
          bookingHistory[i].date
        }</td><td role="button" class="cancel-booking" data-booking-date="${
          bookingHistory[i].date
        }" data-room-number="${bookingHistory[i].roomNumber}" data-room-type="${
          bookingHistory[i].roomType
        }" >Cancel</td></tr>`
      );
    }
    ...
    if (bookingHistory.length !== 0) {
      ...
      $(".rooms-customer-history-table").append(
        `<p class="prompt">No booking history available for the selected customer...</p>`
      );
    }
  },
  ...
  domCustomerServicesHistory(serviceHistory) {
    ...
    let elements = $(
      `<tr><th class="header_hashtag">#</th><th class="room-type">Service</th><th class="available-rooms">Date</th><th class="service-history-cost">Cost</th><tr>`
    );
    ...
    for (let i = 0; i < data.length; i++) {
      elements = elements.add(
        `<tr><td>${i + 1}</td><td>${
          ...
          data[i].date
        }</td><td class="align-center">${data[i].totalCost}</td></tr>`
      );
    }

    elements = elements.add(
      `<tr><td class="align-center" colspan="3">Total</td><td class="align-center">${total}</td></tr>`
    );
    ...
    if (data.length !== 0) {
      ...
      $(".services-customer-history-table").append(
        `<p class="prompt">No service history available for the selected customer...</p>`
      );
    }
  },
  ...
  domAllAvailableRooms(availableRooms) {
    ...
    $(".available-all-rooms-title").text(
