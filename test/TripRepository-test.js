...
describe('TripRepository', () => {
  ...
  it('should get a traveler\'s trips by status', () => {
    expect(tripRepo1.getUpcomingTripsByStatus('approved')).to.deep.equal([
      { id: 17, userID: 2, destinationID: 8, travelers: 1, date: '2022/10/30', duration: 20, status: 'approved', suggestedActivities: [] }
    ]);
    expect(tripRepo1.getUpcomingTripsByStatus('pending')).to.deep.equal([
      { id: 20, userID: 2, destinationID: 5, travelers: 4, date: '2023/10/05', duration: 6, status: 'pending', suggestedActivities: [] }
    ]);

    expect(tripRepo2.getUpcomingTripsByStatus('approved')).to.deep.equal([
      { id: 15, userID: 4, destinationID: 10, travelers: 3, date: '2022/12/04', duration: 6, status: 'approved', suggestedActivities: [] }
    ]);
    expect(tripRepo2.getUpcomingTripsByStatus('pending')).to.deep.equal([
      { id: 22, userID: 4, destinationID: 3, travelers: 4, date: '2023/05/01', duration: 19, status: 'pending', suggestedActivities: [] }
    ]);

    expect(tripRepo3.getUpcomingTripsByStatus('approved')).to.deep.equal([
      { id: 13, userID: 6, destinationID: 12, travelers: 1, date: '2022/11/12', duration: 11, status: 'approved', suggestedActivities: [] }
    ]);
    expect(tripRepo3.getUpcomingTripsByStatus('pending')).to.deep.equal([
      { id: 24, userID: 6, destinationID: 1, travelers: 5, date: '2023/11/15', duration: 7, status: 'pending', suggestedActivities: [] }
    ]);
  });

  it('should get the lodging cost for an individual trip', () => {
    expect(tripRepo1.getCost(destinationData, 'estimatedLodgingCostPerDay', 'duration')).to.equal(900)
    expect(tripRepo2.getCost(destinationData, 'estimatedLodgingCostPerDay', 'duration')).to.equal(2470)
    expect(tripRepo3.getCost(destinationData, 'estimatedLodgingCostPerDay', 'duration')).to.equal(490)
  });

  it('should get the flight cost for an individual trip', () => {
    expect(tripRepo1.getCost(destinationData, 'estimatedFlightCostPerPerson', 'travelers')).to.equal(2600)
    expect(tripRepo2.getCost(destinationData, 'estimatedFlightCostPerPerson', 'travelers')).to.equal(3800)
    expect(tripRepo3.getCost(destinationData, 'estimatedFlightCostPerPerson', 'travelers')).to.equal(2000)
  });

  it('should get a trip\'s total cost', () => {
    expect(tripRepo1.getTotalTripCost(destinationData)).to.equal(3850)
    expect(tripRepo2.getTotalTripCost(destinationData)).to.equal(6897)
    expect(tripRepo3.getTotalTripCost(destinationData)).to.equal(2739)
  });

  it('should get the total cost spent for any given year', () => {
    expect(tripRepo1.getTotalSpentForAnyYear(2022, destinationData)).to.equal(11902)
    expect(tripRepo1.getTotalSpentForAnyYear(2021, destinationData)).to.equal(4035)

    expect(tripRepo2.getTotalSpentForAnyYear(2022, destinationData)).to.equal(8767)
    expect(tripRepo2.getTotalSpentForAnyYear(2021, destinationData)).to.equal(4543)

    expect(tripRepo3.getTotalSpentForAnyYear(2022, destinationData)).to.equal(11055)
    expect(tripRepo3.getTotalSpentForAnyYear(2021, destinationData)).to.equal(1782)
  });

  it('should get the total cost spent for any given year', () => {
    expect(tripRepo1.getTotalSpentForYearToDate( destinationData)).to.equal(8052)
    expect(tripRepo2.getTotalSpentForYearToDate( destinationData)).to.equal(6688)
    expect(tripRepo3.getTotalSpentForYearToDate( destinationData)).to.equal(7920)
  });
});