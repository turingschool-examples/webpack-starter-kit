...
describe('Filter rooms by their room type', function() {
    it.skip('Should return a list of rooms that fit a given room type and that are available on the date chosen', function() {
        const date = "2022/01/10";
        const availableRooms = getAvailableRooms(bookings, rooms, date);
        const type = 'suite'
    ...
    it.skip('Should notify user if there are no rooms by that type available', function() {
        const date = "2022/01/10";
        const availableRooms = getAvailableRooms(bookings, rooms, date);
